 ...
 * 
 */
public class BuildGraph extends AbstractGraph {
	...
	final static Object houseRelationData[][] = {
			// From memberName, To memberName, relation desc
			...
			{ "Napoleon V Victor", "Marie Clotilde Bonaparte", "child" },
	...
	BuildGraph(String[] args) throws Exception
	{
		super(args);
	}

	BuildGraph(TGConnection conn) throws Exception
	{
		super(conn);
	}
	...
	{
		TGNode houseMember;
		HashMap<String, TGNode> houseMemberTable = new HashMap<String, TGNode>();
		...
		for (int i = 0; i < houseMemberData.length; i++) {
			...
			if (houseMemberData[i][5] != null) {
				reignStart.setTime(sdf.parse((String) houseMemberData[i][5]));
			...
			if (houseMemberData[i][6] != null) {
				reignEnd.setTime(sdf.parse((String) houseMemberData[i][6]));
	...
	public static void main(String[] args) throws Exception {
		...
		BuildGraph bg = new BuildGraph(args);
		try {
			bg.build();
		}
